...
def generate_js(prs, image_map, output_path, pptx_filename, warnings):
    ...
    # Header
    lines.append(f"// Generated by deckwright — this script produces {pptx_filename}")
    lines.append(f"// Edit THIS FILE, not the .pptx. Changes to {pptx_filename} will be")
    lines.append("// overwritten the next time this script runs.")
    lines.append("//")
    lines.append('// Run: node ' + os.path.basename(output_path))
    lines.append("")
